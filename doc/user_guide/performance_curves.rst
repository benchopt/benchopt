...
- ``RunOnGridCriterion(grid)`` calls the solver over a fixed grid of values and stops when the grid has been entierly explored. The values in the grid can represent anything for the solver: a number of iterations, a target accuracy, an hyperparameter value, etc...
