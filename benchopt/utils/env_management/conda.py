"""Conda backend for ``benchopt install``.

Holds the conda env-management functions (relocated from
``benchopt/utils/conda_env_cmd.py`` and ``benchopt/utils/shell_cmd.py``)
together with the :class:`CondaBackend` adapter that exposes them via
the :class:`~benchopt.utils.env_management.base.EnvBackend` protocol.

The module-level functions remain available so that any caller that
previously imported them from ``conda_env_cmd`` / ``shell_cmd`` keeps
working through back-compat shims.
"""
import os
import re
import sys
...
def _run_shell_in_conda_env(script, env_name=None, raise_on_error=None,
    ...
    See :func:`benchopt.utils.shell_cmd._run_shell` for parameter details.
    """
...
class CondaBackend(EnvBackend):
    """:class:`EnvBackend` implementation backed by ``conda``.

    Thin adapter that delegates to the module-level functions above. The
    body of each function is unchanged from the pre-refactor
    ``conda_env_cmd`` module, so user-visible behavior is identical.
    """

    name = "conda"

    def can_install(self, requirement):
        # Conda can install anything authors might declare today —
        # conda packages, channel-prefixed packages, and pip:: packages.
        return True
    ...
    def install_packages(self, *packages, env_name=None, quiet=False):
        install_in_conda_env(*packages, env_name=env_name, quiet=quiet)

    def install_shell_script(self, script, *, env_name=None, quiet=False):
        shell_install_in_conda_env(script, env_name=env_name, quiet=quiet)
