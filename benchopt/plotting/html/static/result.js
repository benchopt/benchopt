...
const exportLatex = (button) => {
  ...
  // LaTeX table, so the output matches the current sorting, search filter,
  ...
  if (title) {
    ...
    latex += '\\begin{table}[h]\n\\centering\n\\caption{'
      + escapeLatex(title.replace(/<br\s*\/?>/g, ', ')) + '}\n';
  }
  latex += '\\begin{tabular}{l' + 'c'.repeat(Math.max(columns.length - 1, 0))
    + '}\n\\toprule\n'
    + columns.join(' & ') + ' \\\\\n\\midrule\n'
    + rows.map(r => r.join(' & ') + ' \\\\').join('\n')
    + '\n\\bottomrule\n\\end{tabular}\n';
  if (title) latex += '\\end{table}\n';
