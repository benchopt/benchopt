...
description: >
  ...
  words: contribute, implement, extend, improve, fix, debug, document, skill,
  add feature, add command, add backend, add test, add skill, add doc, CLI,
  temp_benchmark, CaptureCmdOutput, patch_import,
  whats_new, sphinx-build, doc build, lint, continuous integration,
  PR scope, review a benchopt PR. Benchmark *authoring*
...
- [Agent skills](./skills.md) — the three skill tiers and how to add a new usage skill under `benchopt/skills/` (folder-based discovery, packaging, `sync-skills`).
